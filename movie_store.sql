...
--
-- Stand-in structure for view `cart_items_view`
...
(1, 1, 'Bahia de Maracaibo #222-A', 'Col. Paseos de la castellana, León, Gto.', '37549', 'Casa 1');

-- --------------------------------------------------------

--
-- Stand-in structure for view `shipping_addresses_view`
-- (See below for the actual view)
--
CREATE TABLE `shipping_addresses_view` (
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `users_view`  AS SELECT `users`.`user_id` AS `UserId`, `users`.`first_name` AS `FirstName`, `users`.`last_name` AS `LastName`, `users`.`user_firebase_uid` AS `UserFirebaseUID`, `users`.`register_timestamp` AS `RegisterTimestamp` FROM `users` ;
